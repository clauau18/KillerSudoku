...
public class MosaicPanel extends JPanel {
	...
	private static Color colors[] = {Color.CYAN, Color.GREEN, Color.GRAY, Color.MAGENTA, Color.ORANGE, Color.PINK, Color.RED, Color.WHITE, Color.YELLOW, new Color(51,153,255), Color.LIGHT_GRAY, new Color(153,102,0), new Color(255,153,0), new Color(255,204,51), new Color(102,255,102), new Color(255,102,102), new Color(255,255,204), new Color(14,209,69)};
   
   //------------------ private instance variables --------------------
   
   private KillerSudoku game;
    ...
    */
   public MosaicPanel(KillerSudoku ks) {
      this(42,42,16,16,ks);
   }
    ...
    *  and with preferred rectangle height and width both set to 16.
    */
   public MosaicPanel(int rows, int columns, KillerSudoku ks) {
      this(rows,columns,16,16,ks);
   }
    ...
    */
   public MosaicPanel(int rows, int columns, int preferredBlockWidth, int preferredBlockHeight, KillerSudoku ks) {
      this(rows, columns, preferredBlockWidth, preferredBlockHeight, null, 0, ks);
   }
    ...
    */
   public MosaicPanel(int rows, int columns, int preferredBlockWidth, int preferredBlockHeight, Color borderColor, int borderWidth, KillerSudoku ks) {
      this.game = ks; 
	  this.rows = rows;
      ...
      this.setLayout(new GridLayout(9, 9));  // 9x9 GridLayout
      ...
      gridChar = new char[rows][columns];
   ...
   public void setGridSize(int rows, 
      ...
      if (rows > 0 && columns > 0) {
         ...
         char[][] newGridChar = new char[rows][columns];
         if (preserveData) {
   ...
   private void drawSquare(Graphics g, int row, int col, boolean callRepaint) {
      ...
      char ch[] = new char[1];
      ch[0] = gridChar[row][col];
      int nbrCage = 0;
      boolean founded = false;
      boolean isFirst = false;
      for(int i = 0; !founded && i < this.game.cages.size(); i++) {
    	  for(int j = 0; !founded && j < this.game.cages.get(i).cells.size(); j++) {
    		  if(this.game.cages.get(i).cells.get(j).abs - 1 == row && this.game.cages.get(i).cells.get(j).ord - 1 == col) {
    			  nbrCage = i;
    			  founded = true;
    			  if(j == 0)
    				  isFirst = true;
      ...
      if (groutingColor == null || (c == null && !alwaysDrawGrouting)) {
         if (c == null) {
        	g.setColor(colors[nbrCage%colors.length]);
            g.fillRect(x,y,w,h);
            g.setColor(Color.BLACK);
            if(isFirst)
            	g.drawString("" + this.game.cages.get(nbrCage).sum, x + (int)Math.round(colWidth/20), y + (int)Math.round(rowHeight/4));
            g.setFont(g.getFont().deriveFont((float)(30)));
            if(this.game.isSolved())
            	g.drawChars(ch, 0, 1, x + (int)Math.round(colWidth/3), y + (int)Math.round(rowHeight*0.75));
            g.setColor( (c == null)? defaultColor : c );
